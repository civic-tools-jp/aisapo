...
function renderAnalysis(){
  ...
  const activeActions=actionItems.filter(x=>x.value>0),emptyActions=actionItems.filter(x=>x.value<=0);
  const orderedActions=[...activeActions,...emptyActions];
  ...
  el.innerHTML=`
  <div class="panel activity-section today-action-section"><div class="section-heading"><div class="heading-icon activity-icon activity-icon-image"><img src="assets/img/activity/needs-action.webp" alt=""></div><div><h2>今日やること</h2><p>対応が必要な項目を優先順に表示</p></div></div><div class="analysis-actions">
    ${orderedActions.map((x,i)=>action(x,activeActions.indexOf(x)+1)).join('')}
  </div>${activeActions.length===0?'<div class="today-clear-note">✓ 現在、優先対応はありません。</div>':''}${dueToday?`<div class="today-due-note">📅 今日が次回予定の訪問先：<b>${dueToday}件</b></div>`:''}${densest?`<button class="analysis-focus-area" onclick="showAnalysisGridOnMap(${densest.lat},${densest.lng})">📍 未訪問が最も集中している地域：<b>${densest.unvisited}件</b>　地図で見る →</button>`:''}</div>
  ...
  <div class="panel activity-section"><div class="section-heading"><div class="heading-icon activity-icon activity-icon-image"><img src="assets/img/activity/town-progress.webp" alt=""></div><div><h2>地域ごとの訪問状況</h2><p>位置情報をもとに、未訪問が多い地域を表示</p></div></div><div class="analysis-town-list">${gridRows||'<div class="notice">位置情報のあるデータがありません。</div>'}</div></div>
