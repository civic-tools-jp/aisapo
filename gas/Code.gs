...
function normalizeMemberClass_(v){
  const s=String(v||'').trim();
  if(['general_member','operator_member','supporter'].includes(s))return s;
...
function normalizeMemberType_(v){
  const s=String(v||'').trim();
  if(['party_member','supporter','general','unknown'].includes(s))return s;
  const cls=normalizeMemberClass_(s);
  if(cls==='supporter')return'supporter';
  if(cls==='general_member'||cls==='operator_member')return'party_member';
  if(/党員|会員/.test(s))return'party_member';
  if(/サポ|support/i.test(s))return'supporter';
  return'unknown';
}
...
function importContacts_(u,p){
  ...
  const existing=rowsWithRow_(SHEETS.RECORDS),allActiveAreas=rows_(SHEETS.AREAS).filter(a=>truth_(a.active)),allAreas=visibleAreas_(u).filter(a=>truth_(a.active)),branches=rows_(SHEETS.BRANCHES),branchNameById=Object.fromEntries(branches.map(b=>[String(b.branchId),String(b.name||'')]));
  ...
  for(const raw of input){
    const partyId=String(raw.partyId||'').trim(),lastName=cleanImportedPersonName_(raw.lastName),address=String(raw.fullAddress||'').trim(),memberClass=normalizeMemberClass_(raw.memberClass),memberType=normalizeMemberType_(raw.memberType||memberClass);
    if(partyId&&existingPartyIds.has(partyId)){duplicateSkipped++;skipped++;continue;}
    if(!partyId||!lastName||!address){skipped++;failed.push({partyId,lastName,address,memberType,memberClass,category:'invalid',reason:!partyId?'党員IDなし':!lastName?'苗字なし':'住所なし'});continue;}
    const targetArea=detectArea_(address,raw.sourceBranch);if(!targetArea){
      const compact=address.replace(/\s+/g,'');
      const outside=allActiveAreas.find(area=>{const city=String(area.city||'').replace(/\s+/g,''),name=String(area.name||'').replace(/\s+/g,'');return !!name&&compact.includes(name)&&(!city||compact.includes(city));});
      skipped++;failed.push({partyId,lastName,address,memberType,memberClass,category:outside?'outside_area':'invalid',reason:outside?'担当支部の活動エリア外です':'活動エリアを判定できません'});continue;
    }
